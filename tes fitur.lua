...
MainFrame.Size = UDim2.new(0,400,0,700) -- diperbesar agar UI muat semua
...
-- Bagian scrollable untuk Utility dan fitur toggle
local UtilityScroll = Instance.new("ScrollingFrame", Content)
UtilityScroll.Name = "UtilityScroll"
UtilityScroll.Size = UDim2.new(1,0,1,0)
UtilityScroll.Position = UDim2.new(0,0,0,0)
UtilityScroll.BackgroundTransparency = 1
UtilityScroll.BorderSizePixel = 0
UtilityScroll.ScrollBarThickness = 8
UtilityScroll.CanvasSize = UDim2.new(0,0,0,0)
UtilityScroll.VerticalScrollBarInset = Enum.ScrollBarInset.Always
UtilityScroll.ClipsDescendants = true

local utilityListLayout = Instance.new("UIListLayout", UtilityScroll)
utilityListLayout.SortOrder = Enum.SortOrder.LayoutOrder
utilityListLayout.Padding = UDim.new(0,8)

-- Helper agar canvas size auto menyesuaikan
utilityListLayout:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function()
    UtilityScroll.CanvasSize = UDim2.new(0,0,0, utilityListLayout.AbsoluteContentSize.Y + 12)
end)
    ...
    box.FocusLost:Connect(function(enter)
        if enter then
            ...
            if n and n >= 16 and n <= 200 then
                ...
                -- Utility Section (scrollable)
                createSeparator(UtilityScroll, "Utility")
                registerToggle("ESP", "ESP", function(state)
                    if state then enableESP() else disableESP() end
                    updateHUD("ESP", state)
                end).Parent = UtilityScroll
                ...
                -- Auto Teleport UI dan logic (masuk ke UtilityScroll agar ikut scroll)
                local autoTP_Section = createSeparator(UtilityScroll, "Auto Teleport")
                local autoTP_Frame = Instance.new("Frame", UtilityScroll)
